...
impl FundingAddressClaimer {
    ...
    pub async fn sweep_swaps(&self, symbol: &str, swaps: Vec<SwapInfo>) -> Result<Vec<String>> {
        ...
        for chunk in swaps.chunks(chunk_size) {
            match self.batch_claim(chunk).await {
                Ok(_result) => claimed.extend(chunk.iter().map(|swap| swap.id.clone())),
                Err(e) => {
...
mod test {
    ...
    #[tokio::test]
    async fn test_sweep_swaps_empty() {
        let currencies = get_test_currencies().await;
        let claimer = create_claimer_with_mock(currencies);

        let result = claimer.sweep_swaps("BTC", vec![]).await;
        assert!(result.is_ok());
        assert!(result.unwrap().is_empty());
    }
    ...
    #[tokio::test]
    async fn test_sweep_swaps_single_btc() {
        let currencies = get_test_currencies().await;
        let claimer = create_claimer_with_mock(currencies);

        // sweep_swaps should use chunk_size of 1 for BTC
        // With empty swaps, it should return empty vec
        let result = claimer.sweep_swaps("BTC", vec![]).await;
        assert!(result.is_ok());
        assert!(result.unwrap().is_empty());
    }
    ...
    #[tokio::test]
    async fn test_sweep_swaps_batch_lbtc() {
        let currencies = get_test_currencies().await;
        let claimer = create_claimer_with_mock(currencies);

        // sweep_swaps should use chunk_size of 10 for L-BTC
        // With empty swaps, it should return empty vec
        let result = claimer.sweep_swaps("L-BTC", vec![]).await;
        assert!(result.is_ok());
        assert!(result.unwrap().is_empty());
    }
