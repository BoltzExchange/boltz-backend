...
6. `transaction.claim.pending`: This status indicates that Boltz is ready for the creation of a cooperative signature for a keypath spend. Taproot Swaps are not claimed immediately by Boltz after the invoice has been paid. Boltz waits for the API client to post a signature for a key path spend, which is the preferred method as it reduces chain footprint and increases privacy for both parties. If the API client does not cooperate in a key path spend, Boltz includes the claim transaction in a script path spend batch.&#x20;
