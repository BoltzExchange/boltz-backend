...
class ConnectionHelper {
  ...
  public connectByPublicKey = async (
    lndClient: LndClient,
    publicKey: string,
  ): Promise<void> => {
    this.logger.verbose(
      `Trying to connect to LND ${lndClient.symbol} node: ${publicKey}`,
    );
    ...
    if (
      ...
      nodeInfo.node.addressesList.length === 0
    ) {
    ...
    for (const address of nodeInfo.node.addressesList) {
      this.logger.debug(
        `Trying to connect to LND ${lndClient.symbol} addresses: ${publicKey}@${address.addr}`,
      );
      ...
      try {
        ...
        this.logger.debug(
          `Could not connect to to LND ${
            lndClient.symbol
          } address ${publicKey}@${address.addr}: ${formatError(error)}`,
        );
      ...
      this.logger.verbose(
        `Connected to LND ${lndClient.symbol} node: ${publicKey}`,
      );
      return;
    }
