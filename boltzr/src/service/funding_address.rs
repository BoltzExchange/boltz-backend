...
impl FundingAddressService {
    ...
    pub async fn create(
        ...
        let script_pubkey = ScriptPubKey {
            symbol: request.symbol.clone(),
            script_pubkey: funding_address.script_pubkey(&our_key_pair)?.to_bytes(),
    ...
    pub async fn set_signature(&self, request: SetSignatureRequest) -> Result<FundingAddress> {
        ...
        if funding_address.status == FundingAddressStatus::TransactionClaimed.to_string() {
            return Err(anyhow::anyhow!(
                "funding address has already been claimed"
            ));
        }
        let key_pair = self.key_pair(&funding_address)?;
...
mod test {
    ...
    impl TestContext {
        ...
        fn with_insert_success(mut self) -> Self {
            self.funding_helper.expect_insert().returning(|_| Ok(1));
            self
        }
        ...
        where
            F: Fn(&FundingAddress) -> bool + Send + 'static,
        {
            self.funding_helper
                .expect_insert()
                .withf(validator)
    ...
    mod create {
        ...
        async fn uses_correct_key_index() {
            ...
            let service = TestContext::new()
                ...
                .with_insert_validator(move |fa| fa.key_index == expected)
                .with_script_pubkey_success()
                .with_currencies()
                .await
                .build();
