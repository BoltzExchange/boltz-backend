...
The `boltz-cli` tool allows you to interact with a running backend over gRPC. It can be useful for backend development and perform maintenance tasks. It is available on the `bin` folder after compiling the backend.
