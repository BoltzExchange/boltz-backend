...
function startClns () {
  ...
  nohup lightningd --regtest --lightning-dir .lightning2 --bind-addr 127.0.0.1:9738 --grpc-port 9295 --developer --dev-fast-gossip --dev-fast-reconnect --dev-bitcoind-poll=5 > /dev/null 2>&1 & num="0"

  waitForCln "lightning-cli"
  waitForCln "lightning-cli --regtest --lightning-dir .lightning2"
