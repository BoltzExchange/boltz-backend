...
class ReferralStats {
  public static generate = async (referralKey?: string): Promise<
  ...
  > => {
    ...
    data.forEach((ref) => {
      const assets = getNestedObject(getNestedObject(getNestedObject(years, ref.year), ref.month), ref.referral);
