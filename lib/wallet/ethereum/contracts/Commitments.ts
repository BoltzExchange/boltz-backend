...
class Commitments {
  ...
  public commit = async (
    ...
    if (currency === this.network.symbol) {
      const expectedAmount = BigInt(swapAmount) * etherDecimals;
      ...
      const expectedAmount = erc20Wallet.formatTokenAmount(swapAmount);
    ...
    this.logger.info(
      `Creating ${currency} commitment for ${swapTypeToPrettyString(swap.type)} Swap ${swapId}: ${transactionHash}@${event.logIndex}`,
    );
    ...
    if (currency === this.network.symbol) {
      this.eventHandler.emit('eth.lockup', {
        ...
        etherSwapValues: {
          amount: event.amount,
          claimAddress: await this.signer.getAddress(),
  ...
  private findLockupEvent = async (
    ...
    for (const event of eventsToCheck) {
      ...
      lockupsFound.push({
        ...
        timelock: Number(parsedEvent.args.timelock),
