...
class MpayService(MpayServicer):
    _mpay: MPay
    _reset: Reset
    _payments_fetcher: Payments
    _route_stats_fetcher: RouteStatsFetcher

    def __init__(
        self,
        mpay: MPay,
        payments_fetcher: Payments,
        route_stats_fetcher: RouteStatsFetcher,
        reset: Reset,
    ) -> None:
        self._mpay = mpay
        self._reset = reset
        self._payments_fetcher = payments_fetcher
    ...
    def GetRoutes(  # noqa: N802
        ...
        if request.destination != "":
            ...
            destinations = self._route_stats_fetcher.get_destinations()

        routes = {
            dest: routes_to_grpc(
                self._route_stats_fetcher.get_routes(
                    dest, request.min_success, request.min_success_ema
                )
            )
    ...
    def ListPayments(  # noqa: N802
        ...
        if payment_hash != "":
            fetcher = self._payments_fetcher.fetch(payment_hash)
        else:
            fetcher = self._payments_fetcher.fetch_all()
...
class Server(GrpcServer):
    ...
    def _register_service(self) -> None:
        add_MpayServicer_to_server(
            MpayService(self._mpay, self._payments_fetcher, self._route_stats_fetcher, self._reset),
            self._server,
        )
