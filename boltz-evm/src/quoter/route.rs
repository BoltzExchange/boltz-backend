    ...
    Fut: Future<Output = Result<Vec<QuoteCallResult>>>,
{
    ...
    while !pending.is_empty() {
        ...
        pending = Vec::new();
        let results = aggregate_unique(
            &mut aggregate,
            calls.iter().map(|call| call.call.clone()).collect(),
        )
        .await?;
    ...
    Q: RouteQuoter + Sync,
{
    ...
    let quoter_candidates = join_all(
        quoters
            .iter()
            .map(|quoter| async move { (quoter.dex(), quoter.candidates(pairs).await) }),
    )
    .await;

    for (dex, result) in quoter_candidates {
        match result {
            ...
            Err(err) => warn!("{} candidate lookup failed: {:#}", dex, err),
        }
    }
...
async fn aggregate_unique<F, Fut>(
    aggregate: &mut F,
    calls: Vec<QuoteCall>,
) -> Result<Vec<QuoteCallResult>>
where
    F: FnMut(Vec<QuoteCall>) -> Fut,
    Fut: Future<Output = Result<Vec<QuoteCallResult>>>,
{
    ...
    let mut result_indexes = Vec::with_capacity(calls.len());

    for call in calls {
        let index = match unique_indexes.get(&call) {
            Some(index) => *index,
            None => {
                let index = unique_calls.len();
                unique_calls.push(call.clone());
                unique_indexes.insert(call, index);
                index
            }
        };
    ...
    let unique_results = aggregate(unique_calls).await?;
    result_indexes
        .into_iter()
        .map(|index| {
            unique_results
                .get(index)
                .cloned()
                .ok_or_else(|| anyhow!("multicall result missing for quote call {}", index))
        })
        .collect()
}
...
mod tests {
    ...
    #[tokio::test]
    async fn generate_routes_fetches_quoter_candidates_concurrently() {
        let token_a = address!("0000000000000000000000000000000000000001");
        let token_b = address!("0000000000000000000000000000000000000002");
        ...
        let routes = tokio::time::timeout(
            ...
            generate_routes(token_a, token_b, &[], &quoters),
        )
        .await
        .expect("candidate lookups should not be serial");
    ...
    #[tokio::test]
    async fn quote_input_routes_deduplicates_shared_first_hops() {
        let token_a = address!("0000000000000000000000000000000000000001");
        let token_b = address!("0000000000000000000000000000000000000002");
        let token_c = address!("0000000000000000000000000000000000000003");
        let token_d = address!("0000000000000000000000000000000000000004");
        let quoters = vec![MockQuoter {
            dex: Dex::UniswapV3,
            candidates: HashMap::new(),
        }];
        let multicall = MockMulticall::new(vec![
            vec![quote_result(14)],
            vec![quote_result(15), quote_result(16)],
        ]);
        ...
        assert_eq!(
            quotes.iter().map(|quote| quote.quote).collect::<Vec<_>>(),
            vec![U256::from(15), U256::from(16)],
        );
        assert_eq!(*multicall.batch_sizes.lock().unwrap(), vec![1, 2]);
    }
    ...
    #[tokio::test]
    async fn quote_output_routes_deduplicates_shared_last_hops() {
        let token_a = address!("0000000000000000000000000000000000000001");
        let token_b = address!("0000000000000000000000000000000000000002");
        let token_c = address!("0000000000000000000000000000000000000003");
        let quoters = vec![MockQuoter {
        ...
        let multicall = MockMulticall::new(vec![
            vec![quote_result(21)],
            vec![quote_result(41), quote_result(42)],
        ]);
    ...
    impl RouteQuoter for MockQuoter {
        ...
        fn quote_input_call(
            ...
            Ok(mock_quote_call(0, self.dex, token_in, hop, amount_in))
        }
        ...
        fn quote_output_call(
            ...
            Ok(mock_quote_call(1, self.dex, token_in, hop, amount_out))
        }
    ...
    impl RouteQuoter for BlockingMockQuoter {
        fn dex(&self) -> Dex {
            self.dex
        }
        ...
        async fn candidates(
            ...
            while self.started.load(Ordering::SeqCst) < self.wait_for {
    ...
    fn mock_quote_call(
        ...
        let mut data = vec![quote_type, dex_id(dex), dex_id(hop.dex())];
        data.extend_from_slice(token_in.as_slice());
        data.extend_from_slice(hop.token_out().as_slice());

        match hop {
            Hop::UniswapV3(hop) => data.extend_from_slice(&hop.fee.to_be_bytes()),
            Hop::UniswapV4(hop) => {
                data.extend_from_slice(&hop.fee.to_be_bytes());
                data.extend_from_slice(&hop.tick_spacing.to_be_bytes());
            }
        }
