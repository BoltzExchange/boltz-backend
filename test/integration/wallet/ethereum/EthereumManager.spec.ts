import { MaxUint256, Transaction, ZeroAddress } from 'ethers';
...
describe('EthereumManager', () => {
  ...
  describe('getReceivedAmount', () => {
    ...
    test('should get refunded amount for Ether refunds', async () => {
      const contracts = manager['contracts'][1];
      const amount = 99889710000000000n;

      const tx = Transaction.from({
        to: await contracts.etherSwap.getAddress(),
        data: contracts.etherSwap.interface.encodeFunctionData(
      ...
      const res = await manager.getReceivedAmount(tx.unsignedSerialized);
      expect(res?.amount).toEqual(amount);
      expect(res?.token).toBeUndefined();
    });

    test('should get refunded amount for ERC20 refunds', async () => {
      const contracts = manager['contracts'][1];
      const amount = 21_000_000n;
      const token = manager.tokenAddresses.get('USDT')!;

      const tx = Transaction.from({
        to: await contracts.erc20Swap.getAddress(),
        data: contracts.erc20Swap.interface.encodeFunctionData(
