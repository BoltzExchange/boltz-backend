...
class EmailNotifier {
  ...
  constructor(
    ...
    if (!config) {
      this.logger.debug('Email config is missing');
      return;
    }

    if (!config.host || !config.to || !config.from) {
      this.logger.warn(
        'Email notifications enabled but missing required configuration (host, port, to, from)',
      );
      return;
    }
    ...
    try {
      this.transporter = nodemailer.createTransport({
        ...
        port: config.port,
        secure:
          config.secure === undefined ? config.port === 465 : config.secure,
        auth: config.user
          ? {
              user: config.user,
              pass: config.pass,
      ...
      this.logger.info(`Email notifications enabled. Sending to: ${config.to}`);
    } catch (error) {
      this.logger.warn(
  ...
  private sendSuccessEmail = async (
    ...
    try {
      ...
      this.logger.info(
        `Success email sent for swap ${swapDetails.id}: ${info.messageId}`,
      );
    } catch (error) {
      this.logger.error(
        `Error sending success email for swap ${swapDetails.id}: ${formatError(
          error,
        )}`,
      );
    }
  ...
  private sendFailureEmail = async (
    ...
    try {
      const info = await this.transporter.sendMail(mailOptions);
      this.logger.info(
        `Failure email sent for swap ${swapDetails.id}: ${info.messageId}`,
      );
    } catch (error) {
      this.logger.error(
        `Error sending failure email for swap ${swapDetails.id}: ${formatError(
          error,
        )}`,
      );
      throw error;
