...
lightning-cli connect $(lightning-cli getinfo --regtest --lightning-dir .lightning2 | jq -r .id)@127.0.0.1:9738 > /dev/null

openChannel bitcoin-cli \
  "lncli --lnddir=/root/.lnd-btc --rpcserver=127.0.0.1:10011 --network=regtest" \
  $(lightning-cli getinfo --regtest --lightning-dir .lightning2 | jq -r .id) \
  9738 false

echo "Opened channels to CLN"

waitForClnChannel "lightning-cli"
waitForClnChannel "lightning-cli --regtest --lightning-dir .lightning2"
