...
mod test {
    ...
    async fn make_get_signing_details_request(id: &str) -> axum::response::Response {
        setup_router()
            .await
            .oneshot(
                Request::builder()
    ...
    async fn test_get_signing_details_not_found() {
        let res = make_get_signing_details_request("nonexistent_id").await;
        assert_eq!(res.status(), StatusCode::NOT_FOUND);
    ...
    async fn test_full_flow() {
        ...
        let signing_details = make_get_signing_details_request(&funding_address_response.id).await;
