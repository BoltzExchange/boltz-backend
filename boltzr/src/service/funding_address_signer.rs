...
impl FundingAddressSigner {
    ...
    pub fn get_signing_details(
        ...
        let tweak = funding_address.tap_tweak(key_pair)?;
        musig.xonly_tweak_add(&Musig::convert_scalar(&tweak.to_be_bytes())?)?;
...
mod test {
    ...
    fn test_set_signature() {
        ...
        let tweak = funding_address.tap_tweak(&server_key_pair).unwrap();
        musig
