...
impl SwapRestore {
    ...
    #[instrument(name = "SwapRestore::restore_xpub", skip_all)]
    pub fn restore_xpub(
        &self,
        xpub: &Xpub,
        ...
        for from in (0..).step_by(GAP_LIMIT as usize) {
            ...
            trace!(
                "Scanning for restorable swaps from key index {} to {} for {}",
                from,
                to,
                xpub.identifier().to_string()
            );
            ...
            let chain_swaps = self.chain_swap_helper.get_by_data_nullable(Box::new(
                crate::db::schema::chainSwapData::dsl::theirPublicKey.eq_any(keys.clone()),
            ))?;
            let reverse_swaps = self.reverse_swap_helper.get_all_nullable(Box::new(
                crate::db::schema::reverseSwaps::dsl::claimPublicKey.eq_any(keys),
            ))?;

            if swaps.is_empty() && chain_swaps.is_empty() {
                debug!(
                    "Scanned {} keys for restorable swaps for {} and found {}",
                    to,
                    xpub.identifier().to_string(),
                    restorable.len(),
                );
                ...
                &mut chain_swaps
                    ...
                    .map(|s| {
                        ...
                        Ok(RestorableSwap {
                            id: s.id(),
                            kind: s.kind(),
                            send: s.receiving().symbol.clone(),
                            receive: s.sending().symbol.clone(),
                            key_index: Self::lookup_from_keys(
                                &keys_map,
                ...
                &mut reverse_swaps
                    ...
                    .map(|s| {
                        ...
                        Ok(RestorableSwap {
                            id: s.id(),
                            kind: s.kind(),
                            send: s.lightning_symbol()?,
                            receive: s.chain_symbol()?,
                            key_index: Self::lookup_from_keys(
                                &keys_map,
                                s.claimPublicKey.clone(),
                                &s.id(),
                            )?,
                            server_public_key: Self::derive_our_public_key(
                                &secp,
        ...
        restorable.sort_by(|a, b| a.key_index.cmp(&b.key_index));
        Ok(restorable)
    }
...
mod test {
    ...
    fn test_restore_xpub() {
        ...
        let res = restore.restore_xpub(&xpub, None).unwrap();
        assert_eq!(res.len(), 2);
        assert_eq!(
            res[0],
            RestorableSwap {
                ...
                send: swap.chain_symbol().unwrap(),
                receive: swap.lightning_symbol().unwrap(),
                key_index: 0,
                ...
                tree: SwapRestore::parse_tree(&swap.redeemScript.unwrap()).unwrap(),
        ...
        assert_eq!(
            ...
            RestorableSwap {
                ...
                tree: SwapRestore::parse_tree(&chain_swap.receiving().swapTree.clone().unwrap())
                    .unwrap(),
                lockup_address: chain_swap.receiving().lockupAddress.clone(),
                status: chain_swap.swap.status.clone(),
                send: chain_swap.receiving().symbol.clone(),
                receive: chain_swap.sending().symbol.clone(),
                key_index: 11,
    ...
    fn test_transform_transaction(
        ...
        assert_eq!(SwapRestore::transform_transaction(tx_id, vout), res);
    }

    #[test]
    fn test_derive_our_public_key() {
        assert_eq!(
            SwapRestore::derive_our_public_key(
    ...
    fn test_derive_blinding_key() {
        assert_eq!(SwapRestore::derive_blinding_key(
            &get_liquid_wallet(),
            "",
    ...
    fn test_derive_blinding_key_non_liquid() {
        assert!(
            SwapRestore::derive_blinding_key(&get_liquid_wallet(), "", "BTC", "")
                .unwrap()
                .is_none()
        );
    }
    ...
    fn test_lookup_from_keys_no_key() {
        let id = "adsf";
        assert_eq!(
            SwapRestore::lookup_from_keys(&HashMap::new(), None, id)
                .err()
                .unwrap()
                .to_string(),
    ...
    fn test_lookup_from_keys_no_mapping() {
        let id = "adsf";
        assert_eq!(
            SwapRestore::lookup_from_keys(&HashMap::new(), Some("".to_string()), id)
                .err()
                .unwrap()
                .to_string(),
    ...
    fn test_derive_keys() {
        let xpub = Xpub::from_str("xpub661MyMwAqRbcGXPykvqCkK3sspTv2iwWTYpY9gBewku5Noj96ov1EqnKMDzGN9yPsncpRoUymJ7zpJ7HQiEtEC9Af2n3DmVu36TSV4oaiym").unwrap();
        let keys = SwapRestore::derive_keys(
    ...
    fn test_derive_keys_custom_path() {
        let xpub = Xpub::from_str("xpub661MyMwAqRbcGXPykvqCkK3sspTv2iwWTYpY9gBewku5Noj96ov1EqnKMDzGN9yPsncpRoUymJ7zpJ7HQiEtEC9Af2n3DmVu36TSV4oaiym").unwrap();
        let keys =
            SwapRestore::derive_keys(&Secp256k1::verification_only(), &xpub, "m/45/1/0/0", 0, 10)
                .unwrap();
    ...
    #[test]
    fn test_parse_tree() {
        assert!(SwapRestore::parse_tree("{\"claimLeaf\":{\"version\":192,\"output\":\"82012088a91433ca578b1dde9cb32e4b6a2c05fe74520911b66e8820884ff511cc5061a90f07e553de127095df5d438b2bda23db4159c5f32df5e1f9ac\"},\"refundLeaf\":{\"version\":192,\"output\":\"205bbdfe5d1bf863f65c5271d4cd6621c44048b89e80aa79301fe671d98bed598aad026001b1\"}}").err().is_none());
    }
}
