            ...
            AmountIn::Exact(amount) => (
                ...
                vec![
                    ...
                    encode_static_params(vec![
                        ...
                        DynSolValue::Address(recipient),
                        DynSolValue::Uint(ACTION_OPEN_DELTA, 256),
                    ]),
                ],
            ),
            AmountIn::ContractBalance => (
...
mod tests {
    ...
    fn encodes_mixed_v3_v4_route() {
        ...
        assert_eq!(
            decode_v4_take(&decode_v4_params(&execute.inputs[1])[2]),
            (token_c, recipient, ACTION_OPEN_DELTA),
        );
        assert_eq!(
            decode_v3_swap_exact_in(&execute.inputs[0]),
            (
                ACTION_ADDRESS_THIS,
                U256::from(100),
                U256::ZERO,
    ...
    fn encodes_mixed_v4_v3_route() {
        ...
        assert_eq!(
            decode_v4_take(&decode_v4_params(&execute.inputs[0])[2]),
            (token_b, ACTION_ADDRESS_THIS, ACTION_OPEN_DELTA),
        );
        assert_eq!(
            decode_v3_swap_exact_in(&execute.inputs[1]),
            (
                recipient,
                action_contract_balance(),
                U256::from(95),
                PathEncoder::new()
                    .add_token(token_b)
                    .add_hop(500, token_c)
    ...
    fn decode_v4_actions(input: &Bytes) -> Vec<u8> {
        decode_v4_input(input).0
    }

    fn decode_v4_params(input: &Bytes) -> Vec<Vec<u8>> {
        decode_v4_input(input).1
    }

    fn decode_v4_input(input: &Bytes) -> (Vec<u8>, Vec<Vec<u8>>) {
        ...
        let DynSolValue::Array(params) = &values[1] else {
            panic!("expected V4 params");
        };

        let params = params
            .iter()
            .map(|param| {
                let DynSolValue::Bytes(param) = param else {
                    panic!("expected V4 param bytes");
                };
                param.clone()
            })
            .collect();
    ...
    fn decode_v4_take(input: &[u8]) -> (Address, Address, U256) {
        let ty = DynSolType::Tuple(vec![
            DynSolType::Address,
            DynSolType::Address,
            DynSolType::Uint(256),
        ...
        let DynSolValue::Tuple(values) = ty.abi_decode_params(input).unwrap() else {
            panic!("expected V4 take tuple input");
        };

        let DynSolValue::Address(currency) = values[0] else {
            panic!("expected V4 take currency");
        };
        let DynSolValue::Address(recipient) = values[1] else {
            panic!("expected V4 take recipient");
        };
        let DynSolValue::Uint(amount, _) = values[2] else {
            panic!("expected V4 take amount");
        };

        (currency, recipient, amount)
    }

    fn decode_v3_swap_exact_in(input: &Bytes) -> (Address, U256, U256, Vec<u8>, bool) {
        let ty = DynSolType::Tuple(vec![
            DynSolType::Address,
            DynSolType::Uint(256),
            DynSolType::Uint(256),
        ...
        let DynSolValue::Tuple(values) = ty.abi_decode_params(input).unwrap() else {
            panic!("expected V3 swap tuple input");
        };

        let DynSolValue::Address(recipient) = values[0] else {
            panic!("expected V3 recipient");
        };
        let DynSolValue::Uint(amount_in, _) = values[1] else {
            panic!("expected V3 amount in");
        };
        let DynSolValue::Uint(amount_out_min, _) = values[2] else {
            panic!("expected V3 amount out min");
        };
        let DynSolValue::Bytes(path) = &values[3] else {
            panic!("expected V3 path");
        };
        let DynSolValue::Bool(payer_is_user) = values[4] else {
            panic!("expected V3 payer flag");
        };
