    ...
    Fut: Future<Output = Result<HashMap<TokenPair, Vec<T>>>>,
{
    ...
    let mut discovered_values = Vec::with_capacity(uncached_pairs.len());

    for pair in uncached_pairs {
        let pools_for_pair = discovered.remove(&pair).unwrap_or_default();
        discovered_values.push((pair, pools_for_pair));
    }

    let cache_values = discovered_values
        .iter()
        .map(|(pair, pools_for_pair)| (pair.cache_field(), pools_for_pair))
        .collect::<Vec<_>>();
    cache
        .set_multiple(cache_key, &cache_values, Some(ttl_secs))
        .await?;

    for (pair, pools_for_pair) in discovered_values {
        if !pools_for_pair.is_empty() {
            pools.insert(pair, pools_for_pair);
        }
    }
