    ...
    pub async fn new(
        ...
        let tls = ClientTlsConfig::new()
            ...
            .ca_certificate(Certificate::from_pem(
                fs::read_to_string(&config.cln.root_cert_path).with_context(|| {
                    format!(
                        "failed to read {} CLN root cert: {}",
                        symbol, config.cln.root_cert_path
                    )
                })?,
            ))
            .identity(Identity::from_pem(
                fs::read_to_string(&config.cln.cert_chain_path).with_context(|| {
                    format!(
                        "failed to read {} CLN cert chain: {}",
                        symbol, config.cln.cert_chain_path
                    )
                })?,
                fs::read_to_string(&config.cln.private_key_path).with_context(|| {
                    format!(
                        "failed to read {} CLN private key: {}",
                        symbol, config.cln.private_key_path
                    )
                })?,
            ));
