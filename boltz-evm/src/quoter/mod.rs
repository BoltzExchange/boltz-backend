...
mod tests {
    ...
    // Arbitrum WETH holds enough ETH for native-value eth_call simulations.
    const FUNDED_CALLER: Address =
    ...
    #[tokio::test]
    #[serial]
    async fn live_arbitrum_encoded_v4_native_call_executes() {
        ...
        let aggregator = QuoteAggregator::new(
            ...
            Cache::Memory(MemCache::new()),
            provider.clone(),
            Some(config(None, None, Some(v4_config()))),
        )
        .await
        .unwrap();

        let quotes = aggregator
            .quote_input(Address::ZERO, USDC, amount_in)
            .await
            .unwrap();
        let (amount_out, data) = quotes
            .iter()
            .find(|(_, data)| matches!(data.hops.as_slice(), [Hop::UniswapV4(_)]))
            .expect("v4 quote");
    ...
    #[tokio::test]
    #[serial]
    async fn live_arbitrum_encoded_mixed_v4_to_v3_call_executes() {
        let provider = arbitrum_provider();
        let amount_in = U256::from(1_000_000_000_000_000u64);
        let aggregator = QuoteAggregator::new(
            "ARB".to_string(),
