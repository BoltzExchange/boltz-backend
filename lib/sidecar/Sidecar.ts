  ...
  public subscribeRelevantTransaction = () => {
    ...
    this.subscribeRelevantTransactionCall.on(
      ...
      async (transaction: sidecarrpc.RelevantTransaction) => {
        ...
        // TODO: how do we wanna deal with that on testnet/regtest where the sidecar does not have a 0-conf endpoint?
        // Ignore 0-conf unsafe Liquid transaction
        if (
          currencyType === CurrencyType.Liquid &&
          transaction.getStatus() ===
            sidecarrpc.TransactionStatus.TRANSACTION_STATUS_ZERO_CONF_SAFE
        ) {
          return;
        }
